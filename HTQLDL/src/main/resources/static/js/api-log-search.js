...
$('#doSearch').on('click', function() {
    ...
    $.ajax({
        async: false,
        type: 'GET',
        url: origin + "/vasonline/search-api-log?alId="+$('#alId').val() + "&module=" + $('#module').val() + "&startDate=" + $('#startDate').val() + "&endDate=" + $('#endDate').val() + "&creator=" + $('#creator').val() + "\
		 &referenceId= "+ $("#referenceId").val() +"&page=" + pageNumber + "&size=" + pageSize,
        dataType: 'json',
        success: function(data, responseText, status) {
            ...
            requestData.forEach((item) => {
                ...
                htmlDOM += "<tr><td align='center'>"  + jsonItem.alId +"</td><td align='center'>" + jsonItem.module +"</td><td align='center'>" + jsonItem.referenceId + "</td><td align='center'>" + jsonItem.request.replace(/&/g, "&amp;").replace(/>/g, "&gt;").replace(/</g, "&lt;").replace(/"/g, "&quot;") + "</td><td align='center'>" + jsonItem.response.replace(/&/g, "&amp;").replace(/>/g, "&gt;").replace(/</g, "&lt;").replace(/"/g, "&quot;") + "</td><td align='center'>" + jsonItem.created + "</td><td align='center'>" + jsonItem.creator + "</td></tr>";
            })
            htmlDOM += "</tbody>";
