...
$('#doSearch').on('click', function() {
    ...
    if($('#isDefaultSearch').prop('checked')){
        linkPost = "/vasonline/discount-search?policyName="+$('#policyNameSearch').val()+"&isDefault=1&startDate=" + $('#startDateSearch').val() + "&endDate=" + $('#endDateSearch').val() + "\
		&page=" + pageNumber + "&size=" + pageSize;
    }else {
        linkPost = "/vasonline/discount-search?policyName="+$('#policyNameSearch').val()+"&isDefault=&startDate=" + $('#startDateSearch').val() + "&endDate=" + $('#endDateSearch').val() + "\
		&page=" + pageNumber + "&size=" + pageSize;
    }
        ...
        success: function(data, responseText, status) {
            ...
            requestData.forEach((item) => {
                ...
                htmlDOM += "<tr><td>" + jsonItem.policyName + "</td><td>" + jsonItem.startDate +"</td><td>" + jsonItem.endDate +
                    "</td><td>" + jsonItem.description + "</td>";
                htmlDOM += "<td align='center'><button  data-id='" + jsonItem.id + "' data-title = '"+jsonItem.policyName+"' data-target='#modal-command-type-edit' class='action edit btn btn-primary-outline'>\
                ...
                <a href='/vasonline/delete-discount/" + jsonItem.id + "' class='action delete btn btn-primary-outline'>\
...
$(document).on('click',".edit",function () {
    let idDiscount = $(this).data('id');
    let policyName = $(this).data('title');

    $.ajax({
        url: '/vasonline/edit-discount/' + idDiscount,
        type: 'GET',
        success:function(data){
...
$(document).on('click',".detail", function () {
    ...
    $.ajax({
        url: '/vasonline/agency-discount/' + idDiscount,
        type: 'GET',
        success:function(data){
