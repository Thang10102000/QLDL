...
$('#doSearch').on('click', function() {
    ...
    $.ajax({
        async: false,
        type: 'GET',
        url: origin + "/vasonline/amkam-search?agencyCode="+$('#agencyCode').val() + "&agencyName="+$('#agencyName').val()+"&areaId=" + $('#areaId').val() +"&shopCode="+ $('#shopCode').val() +"&page="+ pageNumber + "&size=" + pageSize,
        dataType: 'json',
        success: function(data) {
            ...
            requestData.forEach((item) => {
                ...
                htmlDOM += "<tr><td>" + jsonItem.agencyCode + "</td><td>" + jsonItem.agencyName + "</td><td>" + jsonItem.shopCode +"</td><td>" + jsonItem.areaId +"</td>";
                ...
                <a href='/vasonline/delete-amkam/" + jsonItem.id + "' class='action delete btn btn-primary-outline'>\
...
$("#export").on('click',function () {
    window.location.href = "/vasonline/amkam-export?agencyCode=" + $('#agencyCode').val()+ "&agencyName=" + $('#agencyName').val()+ "&areaId=" + $('#areaId').val();
})
...
$(document).on('click',".edit",function () {
    ...
    $.ajax({
        url: '/vasonline/edit-am/' + idAgency,
        type: 'GET',
        success:function(data){
...
$(document).on("click", ".delete", function(e) {
    e.preventDefault();
    var r = confirm("Bạn muốn xóa AMKAM?");
    if (r==true){
